...
def write(x, img,result):
    ...
    if label == "car" or label == "bus" or label == "train" or label == "truck":
        label = "car"
        c1 = tuple(x[1:3].int())
        c2 = tuple(x[3:5].int())
    ...
    if label == "bicycle" or label == "motorbike" or label == "cyclist":
        label = "cyclist"
        c1 = tuple(x[1:3].int())
        c2 = tuple(x[3:5].int())
        result.append([c1[0],c1[1],c2[0],c2[1]])
...
def arg_parse():
    ...
    parser.add_argument("--video", dest = 'video', help = "Video to run detection upon", default = "kitti2.avi", type = str)
